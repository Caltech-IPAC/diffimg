...
# Subs related to difference-image processing.
...
def reformat_science_fits_file_and_compute_uncertainty_image_via_simple_model(input_filename,
                                                                              hdu_index,
                                                                              fname_output,
                                                                              fname_output_unc,
                                                                              append_extra_col,
    ...
    if append_extra_row:
        new_row_shape = np_data.shape[1]
        print("new_row_shape =",new_row_shape)
        new_row = np.full(new_row_shape, clipped_image_mean)
        for i in range(num_extra_rows):
            new_arr = np.append(np_data, [new_row], axis=0)                 # Append extra row of trimmed-average background.
    else:
        new_arr = np_data.copy()

    if append_extra_col:
        new_col = np.full((new_arr.shape[0], num_extra_cols), clipped_image_mean)
        new_np_data = np.append(new_arr, new_col, axis=1)               # Append extra column of trimmed-average background.
    else:
        new_np_data = new_arr.copy()
...
def compute_diffimage_uncertainty(sca_gain,
    ...
    stats = util.fits_data_statistics_with_clipping(diffimage_filename,n_sigma,hdu_index)
...
def gainMatchScienceAndReferenceImages(filename_sci_image,
    ...
    print("awintobwinworldratioref_val: min,q1,median,q3,max = ",min_awintobwinworldratioref_val,
                                                ...
                                                q3_awintobwinworldratioref_val,
                                                max_awintobwinworldratioref_val)
